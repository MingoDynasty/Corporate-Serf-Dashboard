...
def _use_tmp_preferences(monkeypatch, tmp_path, user_root_codes=frozenset()):
    preferences_path = tmp_path / "data" / "preferences.json"
    monkeypatch.setattr(visibility, "PREFERENCES_FILE_PATH", preferences_path)
    monkeypatch.setattr(
        visibility,
        "get_user_root_playlist_codes",
        lambda: set(user_root_codes),
    )
    visibility.clear_preferences_cache()
...
def test_missing_file_seeds_defaults_plus_user_root_without_writing(
    ...
    preferences_path = _use_tmp_preferences(
        monkeypatch,
        tmp_path,
...
def test_toggle_round_trips_and_reports_new_state(monkeypatch, tmp_path):
    _use_tmp_preferences(monkeypatch, tmp_path)
