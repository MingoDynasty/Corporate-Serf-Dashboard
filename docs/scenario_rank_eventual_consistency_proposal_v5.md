# Scenario Rank Eventual Consistency Proposal (v5)
...
**Scope of the no-regression guarantee.** The invariant holds only because every
writer that could *lower* the cache goes through `_save_rank_if_fresher`, and
therefore the lock. `get_scenario_rank_info` still writes unconditionally and
*without* `_rank_save_lock`
([`api_service.py:949`](../source/kovaaks/api_service.py:949)). That is safe
today: its writes are either read-through backfills of the same cached value or
freshly-fetched server truth (monotonic from KovaaK's), and `force_refresh` has
no caller. The latent trap is the **deferred manual-refresh button** — wiring it
to `get_scenario_rank_info(force_refresh=True)` would let a concurrent
force-refresh bypass both the lock and the monotonic guard and regress a running
loop's higher score. If that button is added, route its write through
`_save_rank_if_fresher` (or have it take `_rank_save_lock`) rather than relying
on the unconditional save while a freshness loop may be in flight.
...
| `resolve_leaderboard_id` returns `None` | Terminal: the scenario has no leaderboard on KovaaK's. Stop the loop with a warning. |
...
- **Manual-refresh button: deferred.** Not built in this change. The 168h rank
  ...
  but note the concurrency caveat under [Monotonic conditional
  save](#monotonic-conditional-save): that path writes unconditionally and
  without `_rank_save_lock`, so to preserve the no-regression guarantee while a
  freshness loop may be in flight, the button's write should go through
  `_save_rank_if_fresher` or take the lock.
