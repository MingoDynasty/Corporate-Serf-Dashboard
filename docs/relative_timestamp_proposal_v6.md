# Relative ("Humanized") Timestamp Proposal — v6

## What changed since v5

- **`now` parameter type made consistent.** v5's snippet defaulted `now` to
  `Date.now()` (a millisecond **number**) but then called `now.getDate()` (a
  **Date** method) — incompatible. v6 makes the injectable parameter `nowMs` (a
  millisecond number, default `Date.now()`) and derives the `Date` objects
  internally. Only `relativeTime` takes it; `absoluteTime` is unaffected. See
  [Shared design](#one-js-helper-in-assets) and
...
[`assets/dashAgGridFunctions.js`](../assets/dashAgGridFunctions.js), whose first
line creates the registry
`var dagfuncs = (window.dashAgGridFunctions = window.dashAgGridFunctions || {});`
— i.e. `dagfuncs` *is* `window.dashAgGridFunctions`. Add the two formatters to
that same registry; **no new global is needed.** The two call sites run in
...
Two functions, both **pure** (no DOM access). `relativeTime` takes an optional
`nowMs` (a millisecond number, default `Date.now()`) so it is deterministically
testable; `absoluteTime` has no time-dependent branch and needs no such param:

- `relativeTime(seconds, sentinel, nowMs?)` → `"5 months ago"` / `sentinel` for
  null. Always a relative string (never an absolute date).
- `absoluteTime(seconds, sentinel)` → full timestamp for the **tooltip**,
  formatted to match the home page's existing `"%Y-%m-%d %I:%M:%S %p"` so both
  pages read identically.

### Relative-time formatting

**Hand-roll the strings; don't use `Intl.RelativeTimeFormat` (decided).** Intl
buys i18n + pluralization, but this app is English-only and you still pick the
unit yourself, so ~15 lines of plain JS in one place is simpler to read and fully
controls the edges. Revisit Intl only if real i18n ever lands.

**The ladder.** Always a **single rounded unit — never compound** ("2 months and
15 days ago" reads too heavy and breaks the convention users know). Relative the
whole way; units coarsen so a large age never reads as a giant number:

```
≤ 60s              → "just now"   (also catches zero/negative diffs — see below)
< 60 min           → "N minutes ago"
< 24 h             → "N hours ago"
< 1 calendar month → "N days ago"
< 12 months        → "N months ago"
≥ 12 months        → "N years ago"
```

- **Inputs are derived once** from the millisecond number, giving both the raw
  diff (for the sub-month buckets) and `Date` objects (for the calendar
  month/year math):
...
- **Minutes is the floor** (never "seconds ago" — too fast to read, and it would
  force a 1s tick). Any diff ≤ 0 (future timestamp from clock quirks) renders
  "just now".
- **Months and years are calendar-based, not day-division** — this matches what
  a human reading two dates would say, and makes the day→month handoff exact
  (played Jan 15 → "N days ago" through Feb 14, then "1 month ago" on Feb 15):

  ```js
  let months = (now.getFullYear() - then.getFullYear()) * 12
             + (now.getMonth() - then.getMonth());
  if (now.getDate() < then.getDate()) months--;   // day-of-month not yet reached
  months = Math.max(0, months);                    // never negative
  const years = Math.floor(months / 12);
  ```

  **Edge cases to clamp and test** (date math is where off-by-ones live): the
  `Math.max(0, …)` guard keeps a near-now or future timestamp from going
  negative; **end-of-month** (played Jan 31, now Feb 28 → the day-of-month
  adjustment yields 0 months → "28 days ago", the conservative/correct answer;
  "1 month ago" appears Mar 1); and **leap years** (Feb 29). See the
  [QA checklist](#testing-impact). Day-division (`floor(days/30.44)`) was
  considered to dodge these but only trades them for boundary fudges
  (`floor(365/365.25) = 0` years at exactly 365 days), so calendar + clamp is
  kept.

**Tail: stay relative (decided).** This field answers "how stale is this?", so a
relative string is the answer; an absolute date would force mental arithmetic in
the months range that matters most. Coarsening to months/years keeps it readable
without an absolute fallback. The exact instant is always in the tooltip if
precision is needed. (The one weak spot — "1 year ago" spans 12-23 months — is
immaterial for a staleness gauge and resolved by the tooltip.) See
[Prior art](#prior-art).
...
verbose and infects every floored bucket ("over 1 hour ago") if applied honestly.

**Absolute format (decided).** `absoluteTime` (tooltip only) uses
`"%Y-%m-%d %I:%M:%S %p"` — matching the home tooltip today — not
`toLocaleString()`. A tooltip must show the exact instant unambiguously;
`toLocaleString()` is locale-dependent and `M/D/Y`-ambiguous. ~10 lines of JS
padding / 12h conversion.
...
sentinel, nowMs?)` / `absoluteTime(seconds, sentinel)`. The grid passes
`"Never"`, the home page passes `"N/A"`.
...
clock quirks). With a signed diff, `nowMs - thenMs` goes negative and a naive
formatter would say **"in 5 minutes."** Handled by the "just now" floor (any diff
under 60s, including ≤ 0) plus the `Math.max(0, …)` month clamp.
...
### Absolute-date tail (option A) — rejected for this field

Switching to an absolute date past ~30 days (GitHub Primer / Atlassian
convention) was the v3 decision. Rejected in v4: those systems switch to absolute
where the timestamp is a *reference point* whose exact date has standalone value.
"Last played" is a *staleness gauge*, where a relative string is the answer and a
date would force mental arithmetic. Kept as an alternative because the cutover is
a one-line change if the preference ever flips.

### Day-division month/year math — rejected

`floor(days / 30.44)` / `floor(days / 365.25)` avoids the calendar day-of-month
adjustment, but trades end-of-month edges for boundary fudges
(`floor(365 / 365.25) = 0` years at exactly 365 days) and drifts from what a
human reading two dates would say. Calendar math + a `Math.max(0, …)` clamp is
clearer and tested instead.
...
+ pluralization, the calendar month/year math, and 12-hour absolute formatting.
Two-part plan:
...
1. **Manual QA checklist (acceptance bar for Phase 1).** Verify each branch:
   ...
   - days; the day→month handoff (played on the *N*th → "N days ago" until the
     same day-of-month next month, then "1 month ago").
   - months pluralization; the `11 months` → `12 months` boundary (latter →
     "1 year ago"); years pluralization.
   - **calendar edges:** end-of-month — played Jan 31 shows "28 days ago" on
     Feb 28, then "1 month ago" on Mar 1; **leap year** — Feb 29 against the
     following Feb 28 / Mar 1.
   - absolute format (tooltip): zero-padding, 12-hour clock incl. **12:00 AM**
     (midnight) and **12:00 PM** (noon); renders in local time.
   - null → correct sentinel: **"Never"** in the grid, **"N/A"** on home.
2. **Keep the helpers pure with an injectable `nowMs`** (a millisecond number) so
   a Node smoke test (`node --test`) is a drop-in if/when we add a JS runner. Not
   standing up a Node toolchain now — disproportionate for ~30 lines in a
   pure-Python project.
...
1. Add `relativeTime(seconds, sentinel, nowMs?)` and `absoluteTime(seconds,
   sentinel)` to `assets/dashAgGridFunctions.js` (the existing `dagfuncs` /
   `window.dashAgGridFunctions` registry). Both guard null → sentinel.
   `relativeTime` derives `diffMs` + `Date` objects from `nowMs`
   (default `Date.now()`) and implements the single-unit ladder (just now →
   minutes → hours → days → months → years, calendar-based with the
   `Math.max(0, …)` clamp; any diff ≤ 0 → "just now"; never compound).
   `absoluteTime` formats `"%Y-%m-%d %I:%M:%S %p"`.
...
4. Add a `clientside_callback`: inputs `[Store.data, relative-time-interval.n_intervals]`,
   ...
   `window.dashAgGridFunctions.relativeTime(value, "N/A")`. Seed `children` with
   an initial sentinel to avoid a blank flash before the first fire.
...
5. **Granularity:** hand-rolled ladder, **single unit always (never compound)** —
   just now (≤ 60s, incl. ≤ 0) → N minutes → N hours → N days → N months →
   N years (months/years calendar-based with clamp). No Intl dependency. No
   "over"/"about" prefix.
6. **Absolute format:** `"%Y-%m-%d %I:%M:%S %p"` (tooltip only), not
   `toLocaleString()`.
7. **Tail:** stay relative all the way (no absolute cutover). This is a staleness
...
- **Weeks unit:** currently skipped (days → months). Could insert "N weeks ago"
  for ~7-30 days if desired.
- **Far-out absolute cap:** if "N years ago" ever feels too vague for ancient
  entries, switch to an absolute date past ~2-3 years (not 1 year — "2 years ago"
  is still readable). Default is no cap.
- **"—" placeholder:** switch the whole Scenario Stats block from "N/A" to a
  dimmed "—" (separate change, not part of this proposal).

## Prior art

Market scan of how relative timestamps are handled, June 2026.

**Consensus on the mechanics** (from the cited design systems):
...
**The tail diverges — and the right choice depends on what the field is for:**

- **Stay relative into weeks/months/years** — **AWS Cloudscape** (a tool design
  system) defines no switch threshold. The same pattern is commonly seen in
  consumer apps (e.g. YouTube, Reddit — observed during research, not part of the
  cited scan). Best where the timestamp is a *recency/staleness gauge* — which is
  exactly our "Last played" field. **This is the model we chose.**
- **Switch to an absolute date** — Atlassian after 7 days, GitHub Primer after
  ~1 month. Best where the timestamp is a *reference point* whose exact date has
  standalone value (issue filed on, comment posted on). Not our case.
- A contrarian view (Smykowski, "Stop using relative date and time") argues for
  absolute-only on accessibility/ambiguity grounds — addressed here by keeping
  the exact date in the tooltip.

Sources (fetched): [GitHub Primer — RelativeTime guidelines](https://primer.style/product/components/relative-time/guidelines/),
