# Scenario Rank Eventual Consistency Proposal (v10)

> v10 fixes two interacting gaps in v9's lazy-staleness + interval-polling design.
> (1) **UNRANKED self-heal.** v9's `_is_lazily_stale` only fired for cached RANKED
> entries, so a cold-cache first-PB `UNRANKED` could stick for the full 168h TTL
> if the loop exhausted — contradicting v9's claim that lazy staleness heals it.
> v10 treats a cached `UNRANKED` *with a known local high* as stale, so it
> rechecks. (2) **Interval polling must not fetch.** v9 said the rank callback
> polls every tick *and* never blocks on a network fetch, but the lazy-staleness
> path falls through to a fetch when `cached < local_high` — so every 1s tick
> could hit KovaaK's in the Dash thread during the window. v10 splits the two by
> trigger source: the **interval tick renders cache-only** (`allow_network=False`),
> and lazy network refresh happens only on dropdown/`do_update` triggers.
>
> (v9 was the big simplification: centralized monotonic write replacing the
> in-flight loop counter, lazy staleness check, shorter `(2,4,8,16,32)` schedule,
> adaptive interval polling. Full deltas in git.)
...
- `allow_network=True` (dropdown / `do_update`): full behavior — cache hit served
  unless `_is_lazily_stale`, in which case fall through and fetch; cache miss also
  fetches. The result persists via `_save_rank_monotonic`, which still rejects it
  if the board is still lagging.
- `allow_network=False` (the interval poll): pure cache read. Serve whatever is
  cached (or `UNKNOWN` on a miss); **never** evaluate staleness and **never**
  fetch. This is how the widget cheaply picks up the loop's background writes.
...
- **The interval tick renders cache-only.** When `ctx.triggered_id` is the
  interval, call `get_scenario_rank_info(..., allow_network=False)`: serve
  whatever is cached and **never** fetch or evaluate staleness. This is the whole
  point — a 1s poll must not do network I/O in the Dash thread. Without this
  split, the lazy-staleness path would fetch on *every* tick while the cache sits
  below the local high during the window (the v9 bug this fixes).
...
| Interval tick while the cache is below the local high (mid-window) | Cache-only render: serves the current cached value, no network. The Timer loop is the only fetcher during the window; the tick just picks up whatever the loop last wrote. (This is the v9 per-tick-fetch bug, fixed.) |
...
**Interval poll is cache-only (`allow_network=False`):** even with a cached value
that *would* be lazily stale (RANKED below local high, or `UNRANKED` + local
high), `get_scenario_rank_info(..., allow_network=False)` serves the cached value
and makes **no** `fetch_scenario_rank` call; on a cache miss it returns `UNKNOWN`
without fetching. Pairs with a `home.py`-level test that the interval-triggered
branch passes `allow_network=False` and the dropdown/`do_update` branch passes
...
3. Add the lazy staleness check to `get_scenario_rank_info`: new
   ...
   parameters. With `allow_network=False`, the function is cache-only (serve the
   cached entry, or `UNKNOWN` on a miss; never evaluate staleness, never fetch).
   With `allow_network=True`, a cache hit falls through to the fetch path when
   `_is_lazily_stale(cached, local_high_score)` (covers RANKED-below-local-high and
   `UNRANKED`-with-local-high). In `home.py`'s rank callback, pass
   `get_high_score(selected_scenario)` and set `allow_network = ctx.triggered_id
   != "interval-component"`.
...
- **UI updates via adaptive interval polling, not SSE or blocking (v9; split by
  trigger v10).** The rank widget re-reads the cache on the existing `dcc.Interval`,
  reflecting the loop's write within ~1s without waiting for another run. The two
  jobs are split by `ctx.triggered_id`: the **interval tick is cache-only**
  (`allow_network=False`, never fetches), while lazy network refresh happens only
  on dropdown/`do_update` triggers. Without that split, the lazy-staleness path
