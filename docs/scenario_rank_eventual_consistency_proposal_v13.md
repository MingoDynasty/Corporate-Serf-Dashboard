# Scenario Rank Eventual Consistency Proposal (v13)

> **v13 fixes two gaps in v12's UI wiring** (caught in review by Claude and
...
> v13 keeps a narrowed `allow_network` for resolution avoidance. Full deltas in
> git.)
...
- **`dcc.Loading`** ([home.py:574](../source/pages/home.py:574)) still wraps the
  rank text: it spins during a real fetch (a selection/`do_update` cache miss, or a
  manual Refresh) and stays quiet on cache-only ticks (no fetch ⇒ no spinner, for
  resolved *and* unresolved scenarios alike).
