# Scenario Rank Eventual Consistency Proposal (v12)

> v12 **removes the lazy staleness check** (and with it the `allow_network` /
> cache-only-branch machinery v10–v11 built around it) and replaces it with a
> **manual refresh button**. Why: a locally-generated high score may *never*
> reach KovaaK's — the user played offline, or the server was down at PB time —
> so `cached < local_high` stays true forever and the lazy check would re-fetch
> on every view, hammering the API for scenarios where re-fetching can never help.
> A user-clicked refresh limits API calls to when the user actually wants fresh
> data. Dropping lazy staleness also removes the *only* reason the interval poll
> ever fetched, so `allow_network` and the cache-only branch are deleted: a plain
> interval read serves from cache (populated on selection) with no per-tick
> network. Net: a deletion that lands back near the clean v9 core, plus a button.
>
...
> built the lazy-staleness + `allow_network` design that v12 now retires. Full
> deltas in git.)
...
def refresh_rank(_, selected_scenario):
    ...
    # click again.
    rank_info = get_scenario_rank_info(selected_scenario, config.kovaaks_username, ...,
                                       force_refresh=True)
...
hammering the divergent cases — and it lets us delete the entire `allow_network` /
cache-only-branch machinery v10–v11 needed (see [UI Behavior](#ui-behavior)).
...
- **Add the interval as an Input.** Add `Input("interval-component", "n_intervals")`
  to [`get_scenario_rank`](../source/pages/home.py:130). The widget re-reads the
...
- **The poll does not fetch — and needs no special flag to avoid it.** Because v12
  has no lazy staleness check, a normal read serves from cache during viewing: the
  scenario's mapping, rank, and total caches were all populated by the selection
  (or initial-load) read, so the interval read is a cache hit with no network. A
  cache *miss* fetches once and populates the entry; the next tick is a hit. No
  path fetches on every tick — which is exactly why v12 can **delete** the v10–v11
  `allow_network` parameter and the dedicated cache-only branch. That machinery
  only existed because the lazy check fetched on every tick; remove the lazy check
  and the root cause is gone.
...
- **Decouple notifications from the poll.** `get_scenario_rank` emits
  ...
  ([home.py:152-157](../source/pages/home.py:152)). Firing those every second would
  ...
  tick. (This is independent of fetching — it's about not re-toasting a cached
  warning on every render.)
- **`dcc.Loading`** ([home.py:542](../source/pages/home.py:542)) still wraps the
  rank text: it spins during a real fetch (a selection/`do_update` cache miss, or a
  manual Refresh) and stays quiet on cache-hit ticks.
- **Optional adaptive cadence.** To trim idle load the interval can be enabled/sped
  ...
  a per-tick cache read is cheap.
...
| UI read callback (`get_scenario_rank`) fires during the freshness window | The poll serves from cache (cache hit, no fetch). A dropdown/`do_update` read on a cache *miss* may fetch the lagging value and *display* it, but its write goes through `_save_rank_monotonic`, which rejects the lower/UNRANKED result — cache not clobbered. No counter or coordination needed. |
| Interval tick (any) | Plain cache read. A cache hit serves the cached value with no network — the scenario's mapping, rank, and total caches were populated by the selection read, so the Timer loop is the only fetcher during the window and the tick just surfaces its latest write. A cache miss fetches once and populates; the next tick is a hit. No per-tick fetching (no lazy-staleness path exists). |
...
**Interval poll does not fetch on a cache hit:** seed a fresh rank cache (and a
fresh total cache); call the rank callback's interval-triggered path; assert it
returns the cached value and `_session_get` is **never called**. (No
`allow_network` flag exists — this is just the normal cache-hit path; the test
guards against a regression where a hit would still fetch, e.g. a stale total.)

**Manual refresh forces a one-shot fetch:** the `refresh_rank` callback calls
`get_scenario_rank_info(..., force_refresh=True)`, which fetches once and persists
through `_save_rank_monotonic`. Assert a single `fetch_scenario_rank`, and that a
returned-lower board value does **not** regress a higher cached score.
...
6. Add the UI polling in `home.py`: add `Input("interval-component", "n_intervals")`
   to `get_scenario_rank` so the widget re-renders from cache each tick, and guard
   the `dash_logger` warning/error emission to fire only on change (not every
   tick). No `allow_network` / `ctx.triggered_id` branching — the read is a normal
   cache-served read.
7. Add the manual refresh button: a `dmc.Button`/`dmc.ActionIcon` (`id="rank-refresh-button"`)
   beside the rank widget and a `refresh_rank` callback that calls
   `get_scenario_rank_info(selected_scenario, ..., force_refresh=True)` and writes
   the result via `_save_rank_monotonic`.
8. Add unit tests: `_save_rank_monotonic`/`_is_forward` rule table, read-path
   no-clobber, interval-cache-hit makes no `_session_get` call, manual-refresh
   one-shot fetch (no regression), error-classification, and exhaustion-message
   cases above.
...
- **Manual refresh button instead of an automatic lazy staleness check (v12).**
  ...
  auto-heals — is worth not hammering the API and lets us delete the `allow_network`
  machinery (next decision). See [the manual refresh
  button](#when-the-cache-stays-stale-the-manual-refresh-button).
...
- **UI updates via interval polling, not SSE or blocking (v9; simplified v12).**
  The rank widget re-reads the cache on the existing `dcc.Interval`, reflecting the
  loop's write within ~1s without waiting for another run. With lazy staleness
  gone, this is just a plain cache-served read — no `allow_network` flag, no
  `ctx.triggered_id` branching, no dedicated cache-only branch (all deleted from
  v10/v11). During viewing the read is a cache hit with no network, because the
  selection read populated the mapping/rank/total caches; nothing fetches on every
  tick because there is no lazy-staleness path left. The warning/error emission is
  still guarded to fire on change, not every tick (that's about not re-toasting a
  cached warning, independent of fetching). SSE is unjustified for a local
  single-user tool.
...
- **Manual-refresh button: now built (v12).** Reversed from the earlier "deferred"
  stance: with lazy staleness removed, the button is the user's way to re-pull a
  stale or divergent rank on demand. It reuses `get_scenario_rank_info(force_refresh=True)`
  (no new backend function), and `_save_rank_monotonic` protects it — the
  force-refresh writes through the same forward-only rule, so it cannot regress a
  running loop's higher score. One-shot, not a freshness loop: by the time a user
  clicks, the board has usually settled, and they can click again if not.
...
- `get_scenario_rank_info` keeps its read/display behavior and signature. The only
  change is internal: its two rank-cache writes now go through `_save_rank_monotonic`
  (forward-only). No new parameters — v12 drops the `local_high_score` /
  `allow_network` additions v10/v11 had proposed.
