# Relative ("Humanized") Timestamp Proposal — v5

## What changed since v4

- **Global export made explicit.** The home clientside callback calls the helper
  via the full path `window.dashAgGridFunctions.relativeTime(...)` (the existing
  registry from `assets/dashAgGridFunctions.js`); the grid calls the bare
  `dagfuncs.relativeTime(...)`. No new bare global. See
  [Shared design](#one-js-helper-in-assets).
- **Prior-art citations tightened.** YouTube/Reddit are flagged as observed
  consumer-app examples, not part of the fetched design-system scan. See
  [Prior art](#prior-art).
- **Calendar date-math hardened.** Added the concrete month/year formula with a
  `Math.max(0, …)` clamp, documented end-of-month / leap-year behaviour, and
  added both to the QA checklist. See [the ladder](#relative-time-formatting) and
...
- `relativeTime(seconds, sentinel, now?)` → `"5 months ago"` / `sentinel` for
...
- **Months and years are calendar-based, not day-division** — this matches what
  ...
  ```js
  let months = (nowY - thenY) * 12 + (nowM - thenM);
...
1. Add `relativeTime(seconds, sentinel, now?)` and `absoluteTime(seconds,
   ...
   days → months → years, calendar-based with the `Math.max(0, …)` clamp; any
   diff ≤ 0 → "just now"; never compound). `absoluteTime` formats
   `"%Y-%m-%d %I:%M:%S %p"`.
