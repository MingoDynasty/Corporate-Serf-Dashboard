# Scenario Rank Eventual Consistency Proposal (v6)

> v6 changes over v5: (1) close the read-path stale-write hole with an in-flight
> loop counter so the UI read-through does not persist a lagging fetch while a
> freshness loop owns a scenario; (2) make unresolved-leaderboard (`None`)
> warning-only, not a user toast; (3) add a unit test for the transient-resolver
> retry branch.
...
- **Lifecycle: increment once, decrement once per chain.**
  `schedule_rank_freshness_refresh` increments before scheduling attempt #0, so
  the window is covered from PB detection (not just from the first attempt at
  +30s). Each chain decrements exactly once, on whichever attempt *terminates*
  it — success, exhaustion, a terminal error, or the broad guard — and never
  when an attempt reschedules. The `_run_attempt` pseudocode enforces this with a
  `rescheduled` flag and a `finally` (see [API Shape](#api-shape)).

The read path's one-line change, at
...
def schedule_rank_freshness_refresh(
    ...
    # its own (possibly lagging) fetch from the moment the PB is detected — not
    # only from the first attempt at +30s. The chain releases this in
    # _run_attempt's finally (see below).
...
- **Read-path suppression while a loop is active (the v6 fix):** with
  `_has_active_rank_loop(username, scenario)` true, call `get_scenario_rank_info`
  on a cache miss and assert it fetches and *returns* the (lagging) rank for
  display but does **not** call `save_scenario_rank` — the rank cache file stays
  unchanged/absent. With no active loop, the same cache-miss path *does* save.
...
2. Guard the read path's cache-miss save: in `get_scenario_rank_info`, wrap the
   `save_scenario_rank(...)` at
   [`api_service.py:949`](../source/kovaaks/api_service.py:949) in
   `if not _has_active_rank_loop(username, scenario_name):`. Leave the cache-hit
   scenario-name backfill and everything else unchanged.
...
4. Replace the body of `_refresh_rank_after_high_score` in `file_watchdog.py`
...
6. Add unit tests for the freshness function, including the monotonic-save,
   read-path-suppression, in-flight-counter lifecycle, error-classification, and
   exhaustion-message cases above.
...
- **The read path is arbitrated against in-flight loops, not just other loops.**
  ...
  arbitration](#read-path-arbitration-in-flight-loop-counter).
...
- `get_scenario_rank_info` keeps its read/display behavior. The *only* change is
  one guarded line: its cache-miss/`force_refresh` save now skips while a
  freshness loop is active for that scenario
  ([read-path arbitration](#read-path-arbitration-in-flight-loop-counter)). It
  still returns the fetched rank for display either way, and the cache-hit
  scenario-name backfill is untouched (it re-writes the same value, so it cannot
  regress).
