# Relative ("Humanized") Timestamp Proposal — v4

## What changed since v3

- **Tail flipped to relative (option B).** Old dates stay relative all the way —
  "5 months ago", "1 year ago", "2 years ago" — instead of switching to an
  absolute date past 30 days. This field is a **staleness gauge** ("how long
  since I touched this?"), so a relative string *is* the answer; an absolute date
  forces today-minus-date arithmetic precisely in the months range that matters
  most. See [the ladder](#relative-time-formatting) and
  [Prior art](#prior-art).
- **No absolute cutover.** The "N days ago vs absolute date" threshold is gone.
  Units coarsen (days → months → years) so the string never reads like "437 days
  ago"; the exact instant remains in the tooltip.
- **Weeks skipped** (days run to ~1 month, then months) — fewer unit
  transitions; "3 weeks ago" isn't more useful than "about a month".
- `absoluteTime` is now **tooltip-only** (no cell fallback).
...
- **Minutes is the floor** (never "seconds ago" — too fast to read, and it would
  ...
  "just now".
- **Months and years are calendar-based, not day-division.** Compute the month
  delta from the year/month components
  (`(nowY-thenY)*12 + (nowM-thenM)`, minus 1 if the day-of-month hasn't been
  reached) and `years = floor(months/12)`. This avoids the drift a `days/30.44`
  fudge introduces, and makes the day→month handoff exact (e.g. played Jan 15 →
  shows "N days ago" through Feb 14, then "1 month ago" on Feb 15).
...
1. Add `relativeTime(seconds, sentinel, now?)` and `absoluteTime(seconds,
   ...
   minutes → hours → days → months → years, calendar-based; any diff ≤ 0 → "just
...
5. **Granularity:** hand-rolled ladder, **single unit always (never compound)** —
   ...
   N years (months/years calendar-based). No Intl dependency. No "over"/"about"
   prefix.
...
- **Stay relative into weeks/months/years** — **AWS Cloudscape** (a tool design
  system) defines no switch threshold; **YouTube** and **Reddit** do the same.
  Best where the timestamp is a *recency/staleness gauge* — which is exactly our
  "Last played" field. **This is the model we chose (B).**
